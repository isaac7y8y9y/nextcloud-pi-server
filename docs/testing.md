...
| `scripts/test-restore-live-runtime.py` | Checks private restore approval binding, expiry, replay denial, and freeze retention on injected promotion failure | Python temporary directory removed by cleanup; no Pi access | `<0.1s` | Direct | PR-direct |

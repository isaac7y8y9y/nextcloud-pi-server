...
| `scripts/test-release-upgrade-freeze.py` | Checks release approval binding, replay/expiry denial, and health-before-release ordering | Python temporary directory removed by cleanup; no Pi access | `<0.1s` | Direct | PR-direct |
...
| `scripts/test-restore-live-runtime.py` | Checks private restore approval binding, expiry, replay denial, interrupted-promotion resume, and freeze retention on injected failure | Python temporary directory removed by cleanup; no Pi access | `<0.1s` | Direct | PR-direct |

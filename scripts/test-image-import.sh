...
grep -Fq -- '--plan-rollback-test' "$IMPORTER"
grep -Fq 'force-health-failure,rollback' "$IMPORTER"
grep -Fq 'docker tag "$id" "$tag"' "$SCRIPT_DIR/lib/image-import-remote.sh"
grep -Fq 'docker image inspect --format '\''{{.Id}}'\'' "$id"' "$SCRIPT_DIR/lib/image-import-remote.sh"
grep -Fq 'docker image inspect --platform "$IMAGE_IMPORT_PLATFORM"' "$SCRIPT_DIR/lib/image-import-remote.sh"
grep -Fq '[[ $# -eq 8 && ( "$1" == apply || "$1" == rollback || "$1" == commit ) ]]' "$SCRIPT_DIR/lib/image-import-remote.sh"
grep -Fq 'IMAGE_IMPORT_ROLLBACK_ARMED=1' "$IMPORTER"
grep -Fq 'image_import_remote_action apply' "$IMPORTER"
grep -Fq 'image_import_recover' "$IMPORTER"
grep -Fq 'Image import cleanup is incomplete; preserve transaction ID' "$IMPORTER"
grep -Fq "'\$NEXTCLOUD_IMAGE_PLATFORM'\" </dev/null" "$SCRIPT_DIR/lib/image-import-lifecycle.sh"
grep -Fq 'service stop </dev/null || true' "$SCRIPT_DIR/lib/image-import-remote.sh"
grep -Fq 'active-record rollback "$IMAGE_IMPORT_ID" </dev/null' "$SCRIPT_DIR/lib/image-import-remote.sh"
grep -Fq 'active-record apply "$IMAGE_IMPORT_ID" </dev/null' "$SCRIPT_DIR/lib/image-import-remote.sh"
grep -Fq 'service start </dev/null' "$SCRIPT_DIR/lib/image-import-remote.sh"
grep -Fq 'rollback_reason=forced-test' "$IMPORTER"
grep -Fq 'Image import forced health-failure rollback passed with consumed approval' "$IMPORTER"
grep -Fq 'image import health rollback or cleanup failed' "$IMPORTER"
